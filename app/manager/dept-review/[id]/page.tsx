...
type DeptAction = 'return' | 'reject' | 'forward'
...
const ACTION_LABELS: Record<DeptAction, { title: string; submit: string; placeholder: string }> = {
  ...
  reject: {
    title: 'דחיית הטיול',
    submit: 'אישור דחייה',
    placeholder: 'פרט/י את סיבת הדחייה...',
  },
  forward: {
...
export default function DeptReviewDetailPage({
  ...
  const submitAction = async () => {
    ...
    try {
      ...
      const newStatus =
        action === 'return' ? 'returned_for_changes' : action === 'reject' ? 'rejected' : 'pending'
      setTrip({
        ...trip,
        status: newStatus,
          ...
          <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto">
              ...
              <Button
                ...
                isLoading={processing && actionDialog.action === 'reject'}
                icon={<XIcon size={18} />}
                className="flex-1 md:w-auto h-12"
              >
                דחה
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-black text-gray-800 flex items-center gap-2">
                ...
                {actionDialog.action === 'reject' && <XIcon size={20} className="text-red-500" />}
