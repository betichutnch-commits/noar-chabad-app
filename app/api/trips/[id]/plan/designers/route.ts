...
type Body = { action: "updateDesigner"; name?: string; phone?: string | null; email?: string | null; address?: string | null };
...
const isMissingDesignerSchema = (message?: string | null) => Boolean(message && message.includes("trip_plan_designers"));
...
export async function GET(_request: Request, { params }: RouteContext) {
  ...
  const { data, error } = await supabase
    .from("trip_plan_designers")
    .select("id, name, phone, email, address")
    .eq("trip_id", id)
    .order("name", { ascending: true });
  if (isMissingDesignerSchema(error?.message)) return NextResponse.json({ ok: true, schemaMissing: true, designers: [] });
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json({ ok: true, schemaMissing: false, designers: data || [] });
}
...
export async function PATCH(request: Request, { params }: RouteContext) {
  ...
  if (body.action !== "updateDesigner") return NextResponse.json({ error: "Unsupported action" }, { status: 400 });
  const name = String(body.name || "").trim();
  if (!name) return NextResponse.json({ error: "שם מעצב הוא שדה חובה" }, { status: 400 });
  const { error } = await supabase.from("trip_plan_designers").upsert(
    {
      trip_id: id,
      name,
